...
class ProductsView extends View
{
	...
	function fetch()
	{
		...
		if(!empty($products))
		{
			...
			$category_features = $this->features->get_features(array('category_id'=>$category->id, 'limit'=>4));

			$cf_ids = array();
		 	foreach ($category_features as $f)
			{
				$cf_ids[] = $f->id;
			}
			...
			{
				//$q = $this->db->placehold("SELECT * FROM __options WHERE feature_id in (?@) AND product_id=?", $cf_ids, $p->id);   
				//$this->db->query($q);
				//$p->options = $this->db->results();
				$options = array();
				$item = array();
				foreach ($category_features as $c)
				{
					$item = $this->features->get_options(array('product_id'=>$p->id, 'feature_id'=>$c->id));
					$options[] = $item[0];
				}
				$p->options = $options;
			}
			...
			$this->design->assign('category_features', $category_features);
		...
		//echo "<pre>"; print_r($category_features); 
