...
class View extends Simpla
{
	...
	public $group;
	public $page;
	public $month_rus = array('', 'января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря');	
	public $time_zone = 7200; // сдвиг времени
	...
	public function __construct()
	{
		...
		else
		{
			...
			// Формируем текущий аукцион
			$current_auction = $this->auctions->get_auction(intval($this->settings->current_auction));
			if (empty($current_auction)) 
			{
				$this->design->assign('no_auction', '1');
				$this->db->query("SELECT date_start FROM __auctions WHERE date_start>NOW() LIMIT 1");
				$next_auction = $this->db->result('date_start');
				$this->design->assign('next_auction', $next_auction);
			}
			$_date = strtotime($current_auction->date_start);
			$_date_end = strtotime($current_auction->date_end);

			// Дата начала
			$_current_date1 = new stdClass;
			$_current_date1->day = 	intval(date("d", $_date));
			$_current_date1->month = intval(date("m", $_date));
			$_current_date1->month_label = $this->month_rus[$_current_date1->month];
			$_current_date1->year = 	intval(date("Y", $_date));
			$_current_date1->time = 	date("h:i", $_date);
			$current_auction->date_start = $_current_date1;

			// Дата конца
			$_current_date2 = new stdClass;
			$_current_date2->day = 	intval(date("d", $_date_end));
			$_current_date2->month = intval(date("m", $_date_end));
			$_current_date2->month_label = $this->month_rus[$_current_date2->month];
			$_current_date2->year = 	intval(date("Y", $_date_end));
			$_current_date2->time = 	date("h:i", $_date_end);				
			$current_auction->date_end = $_current_date2;

			// Токен
			$this->design->assign('token', $_SESSION['session_id']);


			// Текущий аукцион в дизайн
			$this->design->assign('current_auction', $current_auction);
			$this->design->assign('auction_started', $this->settings->auction_started);		
			...
			//$this->design->assign('token', '?auth_token=6753F27D20BF8306600C9DAEBC9A16EB');
			...
			$this->design->smarty->registerPlugin("function", "get_currency",	array($this, 'get_currency_plugin'));
		}
	}
	...
	public function get_currency_plugin($params, &$smarty)
	{
		if(!empty($params['var']))
			$smarty->assign($params['var'], $this->money->get_currencies());
	}
