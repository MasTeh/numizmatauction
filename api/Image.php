...
class Image extends Simpla
{
	...
	public function upload_image($filename, $name, $is_doc = false)
	{
		// Имя оригинального файла
		...
		if ($is_doc) $load_path = $this->config->root_dir.$this->config->doc_dir;
		else $load_path = $this->config->root_dir.$this->config->original_images_dir;
		
		if(in_array(strtolower($ext), $this->allowed_extentions))
		{			
			while(file_exists($load_path.$new_name))
			{	
				$new_base = pathinfo($new_name, PATHINFO_FILENAME);
				if(preg_match('/_([0-9]+)$/', $new_base, $parts))
					$new_name = $base.'_'.($parts[1]+1).'.'.$ext;
			...
			if(move_uploaded_file($filename, $load_path.$new_name))			
				return $new_name;
		}
