...
class ComCategories extends Simpla
{
	...
	public function get_categories($filter = array())
	{
		...
		{
			$query = $this->db->placehold("SELECT category_id FROM __comproducts_categories WHERE product_id in(?@) ORDER BY position", (array)$filter['product_id']);
			$this->db->query($query);
	...
	{
		$query = $this->db->placehold("SELECT product_id, category_id, position FROM __comproducts_categories WHERE product_id in(?@) ORDER BY position", (array)$product_id);
		$this->db->query($query);
	...
	{
		$query = $this->db->placehold("SELECT product_id, category_id, position FROM __comproducts_categories ORDER BY position");
		$this->db->query($query);
		return $this->db->results();
	}	
	...
	public function delete_category($ids)
	{
		...
		foreach($ids as $id)
		{
			...
			if(!empty($category->children))
			{
				...
				$this->db->query($query);
				$query = $this->db->placehold("DELETE FROM __comproducts_categories WHERE category_id in(?@)", $category->children);
				$this->db->query($query);
			}
		}
		unset($this->categories_tree);			
	...
	{
		$query = $this->db->placehold("INSERT IGNORE INTO __comproducts_categories SET product_id=?, category_id=?, position=?", $product_id, $category_id, $position);
		$this->db->query($query);
	}
	...
	{
		$query = $this->db->placehold("DELETE FROM __comproducts_categories WHERE product_id=? AND category_id=? LIMIT 1", intval($product_id), intval($category_id));
		$this->db->query($query);
	}
	...
	private function init_categories()
	{
		...
		$query = $this->db->placehold("SELECT c.id, c.parent_id, c.name, c.description, c.url, c.meta_title, c.meta_keywords, c.meta_description, c.image, c.visible, c.position, COUNT(p.id) as products_count
		                               FROM __categories c LEFT JOIN __comproducts_categories pc ON pc.category_id=c.id LEFT JOIN __products p ON p.id=pc.product_id AND p.visible GROUP BY c.id ORDER BY c.parent_id, c.position");
