...
class Pages extends Simpla
{
	...
	*/
	...
	{	
		...
		$query = "SELECT id, url, header, name, meta_title, meta_description, meta_keywords, body, menu_id, position, visible
		          FROM __pages WHERE 1 $menu_filter $visible_filter ORDER BY position $limit";
	...
	public function get_slovar_pages($filter = array())
	{	
		$menu_filter = '';
		$visible_filter = '';
		$pages = array();
		...
		$query = "SELECT id, url, header, name, meta_title, meta_description, meta_keywords, body, menu_id, position, visible
		          FROM __pages WHERE 1 $menu_filter $visible_filter ORDER BY header";
	...
	public function get_pages_by_symbol($symbol)
	{	
		
		$query = "SELECT * FROM __pages WHERE menu_id=2 AND name LIKE '".$symbol."%' ORDER BY position";
