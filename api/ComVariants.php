...
class ComVariants extends Simpla
{
	/**
	...
	*/
	...
	{		
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, v.currency, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment, v.position
					FROM __comvariants AS v
					WHERE 
					1
					$product_id_filter          
					$variant_id_filter   
	...
	{	
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, v.currency, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment
					FROM __comvariants v WHERE id=?
	...
	{
		$query = $this->db->placehold("UPDATE __comvariants SET ?% WHERE id=? LIMIT 1", $variant, intval($id));
		$this->db->query($query);
		return $id;
	}
	...
	{
		$query = $this->db->placehold("INSERT INTO __comvariants SET ?%", $variant);
		$this->db->query($query);
	...
	public function delete_variant($id)
	{
		if(!empty($id))
		{
			...
			$query = $this->db->placehold("DELETE FROM __comvariants WHERE id = ? LIMIT 1", intval($id));
			$this->db->query($query);
	...
	{
		$query = $this->db->placehold("SELECT attachment FROM __comvariants WHERE id=?", $id);
		$this->db->query($query);
		$filename = $this->db->result('attachment');
		$query = $this->db->placehold("SELECT 1 FROM __comvariants WHERE attachment=? AND id!=?", $filename, $id);
		$this->db->query($query);
